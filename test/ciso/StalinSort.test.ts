...
mocha.describe('CiSo StalinSort', () => {
    mocha.it('purges the not aligned', () => {
        ...
        sort.iterate();
        chai.expect(sort.getValues()).to.deep.equal([4, 3, 6, 1, 2, 5]);
        ...
        sort.iterate();
        chai.expect(sort.getValues()).to.deep.equal([4, 3, 1, 2, 0, 6]);
        ...
        sort.iterate();
        chai.expect(sort.getValues()).to.deep.equal([4, 3, 1, 0, 0, 2]);
        ...
        sort.iterate();
        chai.expect(sort.getValues()).to.deep.equal([4, 3, 1, 0, 0, 0]);
